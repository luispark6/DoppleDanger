...
async def process_video(track):
    with pyvirtualcam.Camera(width=960, height=540, fps=13, fmt=PixelFormat.BGR)  as cam:
        while True:
            try:
                frame = await track.recv()  # calls your custom recv logic
                cam.send(frame)
            except Exception as e:
...
async def run_peer_b():
    ...
    def on_track(track):
        ...
        if track.kind == "video":
            ...
            asyncio.ensure_future(process_video(processed_track))
