...
incoming_audio_chunks = deque()

def process_audio(chunk: bytes) -> bytes:
    # Example: amplify the volume
    int16_audio_c = lz4.frame.decompress(chunk)
    int16_audio =np.frombuffer(int16_audio_c, dtype=np.int16)
...
async def playback_processed_audio():
    ...
    with sd.OutputStream(samplerate=SAMPLERATE, channels=CHANNELS, dtype="float32",
                         blocksize=int(BLOCK_TIME*SAMPLERATE), callback=callback):
        await asyncio.Future()

async def run_peer_b():
    print("Waiting for offer.json from Peer A...")
    while not os.path.exists("./webrtc_macbook/offer.json"):
    ...
    def on_datachannel(channel):
        ...
        def on_message(message):
            ...
            if isinstance(message, bytes):
                ...
                incoming_audio_chunks.append(processed)

            else:
                print(f"⚠️ Received non-bytes message: {message}")

    # Load the offer
    with open("./webrtc_macbook/offer.json", "r") as f:
    ...
    with open("./webrtc_macbook/answer.json", "w") as f:
    ...
    try:
        await asyncio.gather(playback_processed_audio())
